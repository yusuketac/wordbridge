...
(async()=>{
  await test("full script initialization and 40 examples",()=>{const h=harness();assert.match(h.get("coHistoryReport").innerHTML,/対象の履歴がありません/);assert.equal(h.run("MANNER_LIBRARY.length"),40);});
  ...
  await test("backup round-trip, deduplication and legacy import",async()=>{
    ...
    h.run("LS.mannerFavorites=['manner-1']");await h.get("bkExport").fire("click");const json=await h.sandbox.lastBlob.text();assert.equal(JSON.parse(json).ver,6);assert.equal(JSON.stringify(JSON.parse(json).mannerFavorites),'["manner-1"]');
    const r=harness();for(let i=0;i<2;i++){r.get("bkFile").files=[{text:json}];await r.get("bkFile").fire("change");}
    assert.equal(r.run("LS.notes[0].learning.points[0].reason"),p.reason);assert.equal(r.run("LS.history[0].learning.points[0].before"),p.before);assert.equal(r.run("LS.notes.length"),1);assert.equal(r.run("LS.history.length"),1);assert.equal(r.run("JSON.stringify(LS.mannerFavorites)"),'["manner-1"]');
